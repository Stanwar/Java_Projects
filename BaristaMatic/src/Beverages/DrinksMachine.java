...
/*
 *  Main Class which initializes and calls other classes and their functions.
 *  This class will call DrinkMaker functions which will in-turn call Drink which calls ingredients to create a drink. 
 * */
...
public class DrinksMachine {
		...
		while(sc.hasNext()){
			...
			if(sc.hasNextInt()){
				...
				if(drinkNames.length >= command && command > 0){
					...
					if(DrinkMaker.isDrinkAvailable(drinkNames[command-1])){
	...
	public static void getMenu(String[] drinkNames) throws ClassNotFoundException, NoSuchMethodException, SecurityException, InstantiationException, IllegalAccessException {
		...
		for(int i=0;i<drinkNames.length;i++){
			System.out.println(i+1 + "," + drinkNames[i] + ",$" + formatter.format(DrinkMaker.getDrink(drinkNames[i]).getTotalCost()) + "," + DrinkMaker.isDrinkAvailable(drinkNames[i]));
		}
